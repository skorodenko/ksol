...
impl MPRISAction for PropertyUpdate {
    fn queue(self, mpris: Arc<Server<Player>>) -> BoxFuture<'static, Result<(), MPRISActionError>> {
        ...
        Box::pin(async move { mpris.properties_changed(props).await.map_err(|x| MPRISActionError::MPRISError(x.to_string())) })
    }
}
...
impl MPRISAction for Seeked {
    fn queue(self, mpris: Arc<Server<Player>>) -> BoxFuture<'static, Result<(), MPRISActionError>> {
        ...
        Box::pin(async move { mpris.emit(signal).await.map_err(|x| MPRISActionError::MPRISError(x.to_string())) })
    }
}
